...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      ...
      if (user.user_metadata) {
        const { age, gender, preferred_topics, communication_style } =
          user.user_metadata;
        setUserPreferences({
          age: age || null,
          gender: gender || null,
      ...
      if (savedConversations) {
        ...
        const savedConversationId = localStorage.getItem(
          `current_conversation_${user.id}`
        );
        if (
          savedConversationId &&
          parsedConversations.some((conv) => conv.id === savedConversationId)
        ) {
          setCurrentConversationId(savedConversationId);

          // Load messages for this conversation
          const conversationMessages = localStorage.getItem(
            `chat_history_${user.id}_${savedConversationId}`
          );
  ...
  useEffect(() => {
    if (user && currentConversationId) {
      localStorage.setItem(
        `chat_history_${user.id}_${currentConversationId}`,
  ...
  const switchConversation = (conversationId) => {
    ...
    // Load messages for this conversation
    const conversationMessages = localStorage.getItem(
      `chat_history_${user.id}_${conversationId}`
    );
    if (conversationMessages) {
      setMessages(JSON.parse(conversationMessages));
    } else {
  ...
  const formatMessageText = (text) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("**") && part.endsWith("**")) {
        ...
        return (
          <em key={i} className={darkMode ? "text-gray-300" : "text-gray-700"}>
            {content}
          </em>
        );
  ...
  return (
    ...
    >
      ...
      <div
        ...
        } md:w-72 h-full flex-shrink-0 transition-all duration-300 overflow-hidden border-r ${
          ...
          <div className="flex-1 overflow-y-auto py-3">
            {conversations.length === 0 ? (
              ...
              <div className="space-y-2 px-3">
                <h3
                  className={`text-xs uppercase font-semibold px-2 mb-2 ${
                    darkMode ? "text-gray-400" : "text-gray-500"
                  }`}
                >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        >
          <div className="flex items-center">
            ...
            <div className="relative section-dropdown">
              ...
              {showSectionDropdown && (
                <div
                  className={`absolute left-0 mt-2 w-56 rounded-md shadow-lg z-10 overflow-hidden ${
                    darkMode
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`p-2 rounded-md flex items-center ${
                darkMode
                  ? "hover:bg-gray-700 text-indigo-400 bg-gray-800/80"
                  : "hover:bg-indigo-50 text-indigo-600 bg-indigo-50/50"
            ...
            >
              {message.sender === "bot" && (
                <div
                  ...
                  } flex items-center justify-center mr-3 mt-1 shadow-md`}
                >
              ...
              <div
                className={`max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl p-4 rounded-lg shadow-sm ${
                  message.sender === "user"
                    ? darkMode
              ...
              {message.sender === "user" && (
                <div
                  ...
                  } flex items-center justify-center ml-3 mt-1 shadow-md`}
                >
                  <FaUser
                    className={`${
          ...
          {loading && (
            <div className="flex justify-start animate-fadeIn">
              ...
              <div
                ...
                } p-4 rounded-lg flex items-center space-x-3 shadow-sm`}
              >
        ...
        >
          <div
            className={`flex items-center space-x-2 rounded-lg px-4 py-3 border ${
              darkMode
                ? "bg-gray-700 border-gray-600 focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500"
                : "bg-white border-gray-300 focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100"
            } shadow-sm`}
          >
            <input
              type="text"
              ...
              className={`flex-1 py-1 px-2 bg-transparent border-none focus:outline-none ${
                darkMode
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  ...
                  } rounded-full h-8 w-8 flex items-center justify-center transition-colors duration-200`}
                    ...
                    {ageOptions.map((option) => (
                      <button
                        ...
                        className={`py-2 px-3 rounded-lg border ${
                          userPreferences.age === option.value
                            ? darkMode
                  ...
                  >
                    {genderOptions.map((option) => (
                      <button
                        ...
                        className={`py-2 px-3 rounded-lg border ${
                          userPreferences.gender === option.value
                            ? darkMode
                              ? "bg-indigo-600 border-indigo-700 text-white"
                              : "bg-indigo-500 border-indigo-600 text-white"
                            : darkMode
                  ...
                  >
                    {communicationStyles.map((style) => (
                      <button
                        ...
                        className={`py-2 px-3 rounded-lg border ${
                          userPreferences.communicationStyle === style.value
                            ? darkMode
                              ? "bg-indigo-600 border-indigo-700 text-white"
                              : "bg-indigo-500 border-indigo-600 text-white"
                            : darkMode
                    ...
                    {topicsOfInterest.map((topic) => (
                      <div key={topic.value} className="flex items-center">
                        <input
                          ...
                          checked={userPreferences.topicsOfInterest.includes(
                            topic.value
                          )}
                          onChange={() => {
                            setUserPreferences((prev) => {
                              const updatedTopics =
                                prev.topicsOfInterest.includes(topic.value)
                                  ? prev.topicsOfInterest.filter(
                                      (t) => t !== topic.value
                                    )
